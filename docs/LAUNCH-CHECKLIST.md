...
- ⛔ **DEPLOY BLOCKED (do this first):** `npx wrangler deploy` returns `entitlements.not_available [code 10007]` — an *account-level* Cloudflare block (reproduces with only the D1 binding; same config deployed fine earlier today). Check the Cloudflare dashboard (billing/plan/notices) + status.cloudflarestatus.com, then retry after a cooldown. The live site still serves the last-good version.
- ⚠️ **#1 RELIABILITY FIX — coded & committed (74bb608), waiting on deploy:** cold isolates time out on the *first* `/v1/check` (CDP `getSupported` hangs ~20-40s); warm = `402` in ~0.2s. Fix caches the real `getSupported` in **D1** (+12s timeout/stale fallback) so cold isolates skip the CDP round-trip. Ships the moment the deploy block clears.
