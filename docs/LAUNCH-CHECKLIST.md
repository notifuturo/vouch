...
- ✅ **COLD-START FIX LIVE & VERIFIED** (v 7f74ee92): the first `/v1/check` on a cold isolate no longer hangs (8/8 fast, no timeouts) — the real `getSupported` is cached in **D1** (+12s timeout/stale fallback). The earlier `entitlements.not_available [code 10007]` deploy block was a **confirmed Cloudflare incident** (entitlements service, ~15:36–16:30 UTC), now cleared. Also live: PR #414 polish (GET /v1/check → 405; `Cache-Control: no-store`).
